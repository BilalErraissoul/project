...
<script src="https://cdn.jsdelivr.net/npm/swiper@11/swiper-bundle.min.js"></script>
  ...
  @include('style')
    ...
    <div class="swiper-slide">
        <img src="/images/{{ $recherche->image }}" class="d-block w-100" alt="{{ $recherche->name }}">
        <div  class="carousel-caption text-dark position-absolute bottom-0 start-0 end-0 mx-0 mb-0 ">
            <div class="annonce-info">
                <h5>{{ $recherche->name }}</h5>
                <p>{!! Str::words($recherche->description, 15, '...') !!}</p>
            </div>
        </div>
    </div>
@endforeach
...
<div class="col-lg-9 pt-0 ">
                        <div class="mb-0">
                            <div class="mb-0">
           <!-- Liste des recherches Section -->
<div class="event-title d-flex justify-content-between align-items-center">
    <h1 class="mb-4"><i class="fas fa-search"></i>RECHERCHES</h1>
    <a href="{{ route('recherches') }}" class="text-decoration-none text-secondary">Afficher tous</a>
</div>

<!-- Recherches Section -->
@foreach ($recherchesEpingler as $recherche)
<div class="article-item mb-4 p-3 mt-4" style="background-color: #ffffff; border-radius: 10px; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1); border: 1px solid #ccc;">
    <div class="row align-items-center">
        <div class="col-md-5 position-relative overflow-hidden">
            <img src="/images/{{ $recherche->image }}" class="img-fluid rounded" style="width: 100%; height: 150px; object-fit: cover;" alt="{{ $recherche->name }}">
        ...
        <div class="col-md-7">
            ...
            <div class="description-container" style="max-height: 5em; overflow: hidden; position: relative;">
                <p class="description-text" style="margin: 0;">{!! $recherche->description !!}</p>
                <span class="more-indicator" style="position: absolute; bottom: 0; right: 0;"></span>
            </div>
            <div class="d-flex justify-content-between align-items-center" style="margin-top: 10px;">
            <a href="{{ route('recherches.show',['recherches'=>$recherche->id]) }}" class="text-primary">En savoir plus</a>
                <p style="margin-bottom: 0;">{{ \Carbon\Carbon::parse($recherche->created_at)->format('D M d Y H:i') }}</p>
            </div>
        </div>
    </div>
</div>
...
@foreach ($recherchesNonEpingler as $recherche)
<div class="article-item mb-4 p-3 mt-4" style="background-color: #ffffff; border-radius: 10px; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1); border: 1px solid #ccc;">
    <div class="row align-items-center">
        <div class="col-md-5">
            <img src="/images/{{ $recherche->image }}" class="img-fluid rounded" style="width: 100%; height: 150px; object-fit: cover;" alt="{{ $recherche->name }}">
        ...
        <div class="col-md-7">
            ...
            <div class="d-flex justify-content-between align-items-center" style="margin-top: 10px;">
            <a href="{{ route('recherches.show',['recherche'=>$recherche->id]) }}" class="text-primary">En savoir plus</a>
