    ...
    <div class="col-lg-9">
        <div class="card border-0 shadow-lg">
            <div class="card-header  text-white py-3"style="background-color:#001229">
                <h2 class="card-title mb-0">{{ $formation->name }}</h2>
                <p class="card-subtitle text-muted ">{{ $formation->date }}</p>
            </div>
            <div class="card-body">
                <p class="card-text text-justify">{!! $formation->description !!}</p>
                <img src="/images/{{ $formation->image }}" class="img-fluid mb-3 rounded" alt="{{ $formation->name }}">
            </div>
            <div class="card-footer bg-light border-0 py-3">
                <a href="{{ route('formations') }}" class="btn btn-secondary">Retour</a>
            </div>
        </div>
    </div>
</div>
