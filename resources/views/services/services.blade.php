...
<div class="container-fluid bg-gradient">
    ...
    <div class="bg-light">
                                <h2 class="mb-6"><i class="fas fa-compass me-2"></i> Accès Rapide</h2>
                            </div>       
        <ul class="list-group">
...
<div class="col-lg-9 pt-0 ">
            ...
            <h1 class="mb-4"><i class="fas fa-cog"></i>SERVICES</h1>
</div>
@foreach ($servicesEpingler as $service)
    <div class="service-item mb-4 p-3 mt-4" style="background-color: #f8f9fa; border-radius: 10px; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1);">
        
        <div class="row align-items-center">
            <div class="col-md-5 position-relative overflow-hidden">
                <img src="/images/{{ $service->image }}" class="img-fluid rounded" style="width: 100%; height: 150px; object-fit: cover;" alt="{{ $service->name }}">
            </div>
            <div class="col-md-7">
                <h5>{{ $service->name }}</h5>
                <div class="description-container" style="max-height: 5em; overflow: hidden; position: relative;">
                    <p class="description-text" style="margin: 0;">{!! $service->description !!}</p>
                    <span class="more-indicator" style="position: absolute; bottom: 0; right: 0;">...</span>
                </div>
                <div class="d-flex justify-content-between align-items-center" style="margin-top: 10px;">
                    <a href="{{ route('services.show',['service'=>$service->id]) }}" class="text-primary">En savoir plus</a>
                    <p style="margin-bottom: 0;">{{ \Carbon\Carbon::parse($service->created_at)->format('D M d Y H:i') }}</p>
                </div>
            </div>
        </div>
    </div>
