    ...
    <div class="col-lg-9">
        <div class="card border-0 shadow-lg">
            <div class="card-header  text-white py-3"style="background-color:#001229">
                ...
                <p class="card-subtitle text-muted ">{{ $event->date }}</p>
            </div>
            <div class="card-body">
                <p class="card-text text-justify">{!! $event->description !!}</p>
                <img src="/images/{{ $event->image }}" class="img-fluid mb-3 rounded" alt="{{ $event->name }}">
            </div>
            <div class="card-footer bg-light border-0 py-3">
                <a href="{{ route('events') }}" class="btn btn-secondary">Retour</a>
            </div>
        </div>
    </div>
</div>
